...
fRetrieveSinglePass = Frame(root)
fRetrieveAllPass = Frame(root)
...
def replacePassYes(window):
    ...
    c.execute('UPDATE sPasswords SET Password = "{}" WHERE Account = "{}"'.format(replacedPass[0], aAccName_text))
...
retrievePass_btn = Button(
    fStart, text="Retrieve Passwords", command=lambda: raise_frame(fRetrievePass))
...
# Select how to Retrieve passwords page

# Creating Back Button
fRetrieve_back_btn = Button(
    fRetrievePass, text="Back", command=lambda: raise_frame(fStart))
fRetrieve_back_btn.grid(row=0, column=0)
# Create Retrieve passwords button
singleRetrievedPass_btn = Button(
    fRetrievePass, text="Retrieve password of\na single account", command=lambda: raise_frame(fRetrieveSinglePass))
singleRetrievedPass_btn.grid(row=1, column=0)
# Create Save a password button
allRetrievedPass_btn = Button(fRetrievePass, text="Retrieve all passwords",
                              command=lambda: raise_frame(fRetrieveAllPass))
allRetrievedPass_btn.grid(row=1, column=1)

# Retrieving a single password page

# Creating Back Button
fRetrieveSinglePass_back_btn = Button(
    fRetrieveSinglePass, text="Back", command=lambda: raise_frame(fRetrievePass))
fRetrieveSinglePass_back_btn.grid(row=0, column=0)
# Creating Labels for Retrieving a single
retrieveAccName_label = Label(fRetrieveSinglePass, text="Enter Account Name: ")
retrieveAccName_label.grid(row=1, column=0)
# Creating text boxes for Saved Passwords Page
retrieveAccName_entry = Entry(fRetrieveSinglePass, width=30)
retrieveAccName_entry.grid(row=1, column=1)
retrieveAccName_text = retrieveAccName_entry.get()
# Creating Enter button
rEnter_btn = Button(fRetrieveSinglePass, text="Enter")
rEnter_btn.grid(row=2, column=1)

# Retrieving all passwords page
fRetrieveAllPass_back_btn = Button(
    fRetrieveAllPass, text="Back", command=lambda: raise_frame(fRetrievePass))
fRetrieveAllPass_back_btn.grid(row=0, column=0)
